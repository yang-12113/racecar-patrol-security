...
namespace lslidar_driver
{
	namespace
	{
	bool is_m10_family(const std::string &lidar_name)
	{
		return lidar_name == "M10" || lidar_name == "M10_GPS" || lidar_name == "M10_P" ||
		       lidar_name == "M10_PLUS" || lidar_name == "M10_DOUBLE";
	}

	bool is_difop_packet(const unsigned char *packet_bytes, int len)
	{
		return len == 188 && packet_bytes[2] == 0x55 && packet_bytes[3] == 0x00 &&
		       packet_bytes[186] == 0xFA && packet_bytes[187] == 0xFB;
	}

	bool has_enough_valid_points(
		...
		else
		{
			...
			else
			{
				while (true)
				{
					...
					if (is_m10_family(lidar_name) && compensation)
					{
						if (is_difop_packet(packet_bytes, len))
							difop = true;
					}
					if (!difop && is_m10_family(lidar_name) && !has_enough_valid_points(packet_bytes, data_bits_start, package_points, 2, 4))
						continue;
					break;
				}
			}
