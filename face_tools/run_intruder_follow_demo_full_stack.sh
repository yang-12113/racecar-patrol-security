...
ros2 launch racecar Run_car.launch.py > /tmp/run_car_intruder_follow.log 2>&1 &
BASE_PID=$!

sleep 4
pkill -f rviz2 2>/dev/null || true

/root/face_tools/run_face_follow_stack.sh --follow-policy unknown --follow-unknown-min-frames 8 "$@" &
STACK_PID=$!
wait "$STACK_PID"
